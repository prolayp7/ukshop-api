...
describe('Admin Media (e2e)', () => {
  ...
  it('uploads, lists, updates, and removes an owner image', async () => {
    ...
    const uploaded = await request(app.getHttpServer()).post('/api/v1/admin/media').set('Authorization', `Bearer ${token}`).field('ownerType', 'CATEGORY').field('ownerId', String(categoryId)).field('collection', 'hero').field('altText', 'Test image').attach('file', png, { filename: 'test.png', contentType: 'image/png' }).expect(201);
    mediaId = uploaded.body.data.id; expect(uploaded.body.data.url).toMatch(/^\/uploads\/.+\.png$/);
